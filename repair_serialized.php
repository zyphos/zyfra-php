...
class RepairSerialize{
	...
	private function _parse($back_ref=null){
		...
		$c = $this->txt[$this->pos++];
		if(is_null($back_ref)) $back_ref = array();
		$obj = null;
		switch($c){
			case 'N':
				$obj = null;
	...
	private function _parse_array($back_ref){
		...
		while ($i<$size && $this->pos < $this->strlen){
			$new_back_ref = array_merge($back_ref,array('[]'));
			$key = $this->_parse($new_back_ref);
			$new_back_ref = array_merge($back_ref,array('['.$key.']='));
	...
	private function _parse_object($back_ref){
		...
		$i = 0;
		while ($i<$size && $this->pos < $this->strlen){
			$new_back_ref = array_merge($back_ref,array('object('.$obj_name.')'));
			$key = $this->_parse($new_back_ref);
			$new_back_ref = array_merge($back_ref,array('object('.$obj_name.')->'.$key));
			$res->$key = $this->_parse($new_back_ref);
			$i++;
		}
		$this->pos++; // }
